...
def main():
    ...
    validate_dataset = get_trainval_datasets(config)
...
def test(**kwargs):
    ...
    with torch.no_grad():
        for i, (X, y) in enumerate(data_loader):
            ...
            #print(y_pred_aux[0])
            species_probs.extend(F.softmax(y_pred_aux, dim=-1).tolist())
...
#         df_family = pd.DataFrame(family_probs, columns=['family_' + x for x in level_names_dict['family']])
    ...
    df_res.to_csv(os.path.basename(config.test_csv.replace('.','_res.')), encoding='utf-8-sig',index=False)
